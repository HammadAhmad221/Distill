...
//     try {
//       // Replace with your transcription API endpoint
//       const response = await axios.post('http://16.171.144.18/audio_transcript', formData, {
//         headers: {
...
//       // Handle the response (display it, navigate, etc.)
//     } catch (error) {
...
//       <button onClick={()=>setFile(null)} className="bg-gray-200 text-[#667091] border border-[#e2e2fd] px-4 py-2 rounded-lg">Cancel</button>
//       <button onClick={handleGenerate} className="bg-[#3F3EED] text-white px-4 py-2 rounded-lg flex items-center gap-2"><img src="/magic.svg" alt="m" />
//         Generate</button>
//     </div>
// </div>  ):(
        ...
        <main className="w-full mx-auto p-4 flex flex-col gap-4 font-space">
          <div className="flex flex-col bg-[#3F3EED] p-4 gap-4 rounded-lg">
            ...
            <div className="text-white text-base font-normal">
              Upload your audio, get transcripts, and unlock key insights effortlessly.
            </div>
            <button className="py-2 bg-white rounded border border-[#9796e9]/10 backdrop-blur-[25px] justify-center items-center inline-flex w-[150px] text-[#3F3EED] font-semibold">
              Get Started
            </button>
            ...
            <div className="flex mb-4 p-2 font-semibold shadow-lg rounded-full">
              <button
                ...
                className={`px-4 py-2 rounded-full flex gap-2 items-center ${
                  isUploadVoice ? 'bg-[#3F3EED] text-white' : 'bg-white text-[#3F3EED]'
                }`}
              >
                {isUploadVoice ? <img src="/magic.svg" alt="" /> : <img src="/magicB.svg" alt="" />}
                Upload Voice
              </button>
              <button
                onClick={handleToggle}
              ...
              >
                {!isUploadVoice ? <img src="/urlW.svg" alt="" /> : <img src="/url.svg" alt="" />}
                URL
              </button>
            </div>
          </div>
...
<div className="flex items-center justify-between py-2 px-6">
                <div className="flex items-center gap-4">
                  <img src="/voice.svg" alt="V" />
                  <div className="text-black text-base">{uploadedFileName}</div>
                </div>
